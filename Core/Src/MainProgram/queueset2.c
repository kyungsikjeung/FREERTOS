...
static QueueHandle_t xCharPointerQueue = NULL;
static QueueHandle_t xUint32tQueue = NULL;
static QueueHandle_t xBinarySemaphore = NULL;
static QueueSetHandle_t xQueueSet = NULL;

void vCharPointerSender(void *pvParams) {
    const TickType_t delay = pdMS_TO_TICKS(500);
    const char * const pcMessage = "Hello from CharPointerQueue!\r\n";
...
/* --------- Sender Task: 숫자 --------- */
void vUint32Sender(void *pvParams) {
    const TickType_t delay = pdMS_TO_TICKS(1000);
    uint32_t value = 0;

    for (;;) {
        vTaskDelay(delay);
        xQueueSend(xUint32tQueue, &value, 0);
        value++;
    }
}

/* --------- Sender Task: 세마포어 --------- */
void vSemaphoreNotifier(void *pvParams) {
...
void vUnifiedReceiver(void *pvParams) {
